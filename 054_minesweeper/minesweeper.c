...
board_t * makeBoard(int w, int h, int numMines) {
  board_t * bt = malloc(sizeof(*bt));
  if (bt == NULL) {
    perror("Cannot allocate heap memory!");
  }
  bt->totalMines = numMines;
  bt->height = h;
  bt->width = w;
  int ** b = NULL;
  int * row = NULL;
...
int countMines(board_t * b, int x, int y) {
  ...
  int count = 0;
  while (h0 < y + 2 && h0 < b->height) {
    int w0 = x - 1;
    if (w0 < 0) {
      w0 = 0;
    }
    while (w0 < x + 2 && w0 < b->width) {
      if (IS_MINE(b->board[h0][w0])) {
        count++;
      }
      w0++;
    }
